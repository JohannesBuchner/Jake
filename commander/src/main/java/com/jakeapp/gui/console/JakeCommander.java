...
import com.jakeapp.violet.actions.project.connect.InviteUserAction;
import com.jakeapp.violet.actions.project.connect.SuggestUsersToInviteAction;
import com.jakeapp.violet.actions.project.connect.UserInfo;
...
public class JakeCommander extends Commander {
	...
	class GoOfflineCommand extends LazyNoParamsCommand {
		...
		protected void handleArguments() {
			try {
				AvailableLaterWaiter.await(new GoOnlineAction(user, null,
						getHost(user), -1, true, loginView));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	class GoOnlineCommand extends LazyNoParamsCommand {
		...
		protected void handleArguments() {
			...
			try {
				AvailableLaterWaiter.await(new GoOnlineAction(user, pw,
						getHost(user), -1, false, loginView));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		...
		public void handleArguments() {
			try {
				...
				Collection<UserInfo> users = AvailableLaterWaiter
						.await(new SuggestUsersToInviteAction(project
								.getModel()));
	...
	private static String getHost(User user) {
		return user.getUserId().substring(user.getUserId().indexOf("@") + 1);
	}
	...
	private final UserOrderStrategy userorder = new SimpleUserOrderStrategy();

	private final Passwords wallet = new JsonPasswords(new File(
			"jake.passwords"));
	...
	protected void onStartup() {
		...
		// DI.register(Projects.class, new JsonProjects(new
		// File("jake.projects")));
