...
//        }
	...
	public Boolean getDeleteState(ILogable belongsTo) {
		...
		//List<LogEntry<? extends ILogable>> le = new LinkedList<LogEntry<? extends ILogable>>();
		/*le.addAll(findMatching(len));
		le.addAll(findMatching(led));
		for(entry : le) {
	 ...
	 */
	private Collection<LogEntry<? extends ILogable>> findTwoMatching(LogAction a, LogAction b) {
		// TODO
		return new LinkedList<LogEntry<? extends ILogable>>();
	}
	/**
	 * finds all that match any of the two Logactions and belong to the given element, sorted by timestamp
	 */
	private Collection<LogEntry<? extends ILogable>> findTwoMatchingFor(LogAction a, LogAction b, ILogable belongsTo) {
		// TODO
		return new LinkedList<LogEntry<? extends ILogable>>();
	}
	...
	public Collection<ProjectMember> getCurrentProjectMembers() {
		...
		for(ProjectMember member : people.keySet()) {
			if(people.get(member).size() == 0)
				people.remove(member); // member isn't trusted by anyone -> removed
		}
