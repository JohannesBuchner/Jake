...
%(*! dominiK: where are the notes to the files? if you decide to not make notes to files, then don't mention that here. I want notes to files! 
% *)
...
%(*! dominik: this is not true afaik. As long as I don't add the new user to my trust list, the relation between me and the new user is not transitive. I will get his files although, but the relation is not transitive. Its also useless to mention reflexive because theres no need to trust oneself. I would remove the term equivalence relation completly for the sake of easyness to understand, especially when open source developers should join the project. 
% *)
\item[member list] the member list is a set of \emph{project members} a user explicitly trusts.
%(*! Dominik: as stated somewhere below in this document, I think you swapped "web of trust" and member list. 
% web of trust are the users i explicitly (and maybe also implicitly) trust. in the member list, every member
% i can get aware of should be listed
% *)
...
%(*! Dominik: the members i explicitly trust... *)
...
%(*! dominik: thats not true imo. the web of trust forms the graph of the members trusting each other, it does not mean that every member trusts each other member. 
% *)
...
%(*! dominik: thats not true. each instance of jake should be limited to one user-session per machine. if i have an multi-user unix system it should of course be possible to have multiple instances of jake running by different users (e.g. by running a remote X session). We just want to prevent a user within the same session to start jake a second time. this can easily be done with some sort of lockfile. Also the ports jake is using MUST NOT be fixated globally as this would introduce problems in this multi-user environment (port already bound) 
% *)
Within this single instance many projects are supported. The number of concurrent \emph{projects} MAY be limited
%(*! dominik: I don't see a reason why we should do that. With proper programming the only limiting resource we have is the amount of ram the user has on his disposal.. but if you want to stick with that, I'm ok with it. 
% *). 
...
%(*! Dominik: I think we should specify here that we really just save which project was opened and nothing more, because an "application state" is a lot more than just a set of opened projects... actually its every object in the jvm at the moment just before jake was closed and thats nothing we want to do. I know you just mean the list of projects, but then better use another term please 
% *)
...
%(*! Dominik: You are aware of the fact, that this reproduces the functionality of the OS trash that we cannot really rebuild with the amount of time we have. Also one has to create the corresponding GUI elements for browsing the trash. I would propose to not delete files deleted by other users, but simply mark them as deleted and show them e.g. in a different color in the users file list and let the user decide if he really also wants to delete the file from his project folder. So we don't have to rebuild the OS Trash or implement our own. The only thing we have to do is to mark the file as deleted by user x in our database and maybe rename it to oldfilename.oldextension.currenttimestamp so there will be no problems if a user deletes a file and then creates a new one with the same name. For the sake of usability we still can do a "delete all deleted files" function or even create an explicit view for the deleted files, but I'm strictly against moving them to the OS Trash or another folder. If you want, you could also rename it to something like "_JAKEDEL_{currenttimestamp}_OLDFILENAME.OLDEXTENSION" and the file would be still usable the same way directly within the OS File Browser. 
% *)
...
%(*! Dominik: IF the user has activated this. If a user has a lot of projects or a project with a great amount of files, we may not want a daemon periodically checking the users files for changes. This should only be done if the user has activated it (or not deactivated it, depends on default value). There has to be a way to disable automatic discovery while jake is running and start the scan manually, e.g. by the refresh icon we already have in the gui 
% *)
