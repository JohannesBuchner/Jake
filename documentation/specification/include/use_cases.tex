...
\def\ucpostconditions{The user is logged in and able to use all features.}
...
\item The project is created and added to the project list.
\item Any files which already exist in the local folder become local files of this project.
\end{enumerate}
}
\def\ucalternativepath{-}
\def\ucpostconditions{The project exists locally.}
\def\ucexceptions{
...
\def\ucsummary{A user may be invited to a project by another user. This process happens via invitations.}
\def\ucpreconditions{The user is logged in (NET-01).}
...
\def\ucprimaryscenario{
\begin{enumerate}
\item The user chooses to open project.
\item The user is asked to select the project in the local filesystem.
...
\def\ucstatus{NOT IN SRS! Do we really need this?} % will be omitted in the final version !!!
...
\def\ucpreconditions{Jake is running (GEN-01) and at least one project is open and active}
\def\uctriggers{User action (active)}
\def\ucprimaryscenario{
\begin{enumerate}
...
\def\ucalternativepath{If auto add/remove is disabled for the sending project member in this project, the user will be notified of this event, but the new member will not be added to the list of members.}
\def\ucpostconditions{The new member is in the local project member list (if auto add/remove is enabled)}
\def\ucexceptions{-}
\def\ucfrequency{Sometimes/weekly}
\ucprint
...
\def\ucalternativepath{If no files exist in the project (yet), this should be made obvious to the user and the process of adding files should be explained.}
...
\def\ucexceptions{If a file doesn't conform to the Jake naming standards, the user will be prompted if they want to rename the file (see FIM-11)}
\def\ucfrequency{Daily/Often}
\ucprint
...
\def\ucprimaryscenario{
\begin{enumerate}
\item New files are added to the project view and files that no longer exist are removed from it.
\end{enumerate}
}
\def\ucalternativepath{-}
...
\def\ucexceptions{If the files can't be deleted for some reason (e.g. access violation because they are still in use by some other program), the user is informed of this and it is suggested that they close the program and/or try to restart their computer and manually delete the files.}
\def\ucfrequency{Daily/Often}
\ucprint
...
\def\ucexceptions{If the file is already tagged with a tag of the same name, the new tag is ignored.}
\def\ucfrequency{Very Often}
\ucprint
...
\def\ucalternativepath{-}
\def\ucpostconditions{The file is no longer locked and other users, if their Jake client is aware of the unset lock, no longer receive lock notifications.}
\def\ucexceptions{-}
\def\ucfrequency{Sometimes}
\ucprint
...
\def\ucsummary{Notes are text associated with projects. They can only be accessed from within Jake.}
\def\ucpreconditions{Jake is running (GEN-01) and a project is open, active and selected}
\def\uctriggers{User action (active)}
\def\ucprimaryscenario{
\begin{enumerate}
...
\def\ucsummary{Editing a note changes its contents.}
\def\ucpreconditions{Jake is running (GEN-01) and a project is open, active and selected and contains at least one note.}
\def\uctriggers{User action(active)}
\def\ucprimaryscenario{
\begin{enumerate}
...
\def\ucsummary{The user may add a tag to a note. Only one note at a time.}
\def\ucpreconditions{Jake is running (GEN-01) and a project is open, active and selected}
\def\uctriggers{User action (active)}
\def\ucprimaryscenario{
\begin{enumerate}
...
\def\ucalternativepath{-}
\def\ucpostconditions{The note is tagged with a tag of the chosen name}
\def\ucexceptions{If the note is already tagged with a tag of the same name, the new tag is ignored.}
\def\ucfrequency{Very Often}
\ucprint
...
\def\ucsummary{The user may remove a tag from a note. Only one tag from one note at a time.}
...
\def\ucalternativepath{-}
\def\ucpostconditions{The note is no longer tagged with a tag of that name}
\def\ucexceptions{-}
\def\ucfrequency{Very Often}
\ucprint
...
\def\ucstatus{Review required (esp: I'm not sure what to do with not downloaded remote notes)} % will be omitted in the final version !!!
...
\def\ucalternativepath{-}
\def\ucpostconditions{The note is locked and other users, if their Jake client is aware of the lock, receive some sort of indication of this fact.}
\def\ucexceptions{-}
\def\ucfrequency{Sometimes}
\ucprint
...
\def\ucalternativepath{-}
\def\ucpostconditions{The note is no longer locked and other users, if their Jake client is aware of the unset lock, no longer receive lock notifications.}
\def\ucexceptions{-}
\def\ucfrequency{Sometimes}
\ucprint
...
\def\ucsummary{Project preferences are project properties that can be changed by the user (e.g. auto-pull/auto-announce)}
\def\ucpreconditions{Jake is running (GEN-01) and a project is open, active and selected}
\def\uctriggers{User action (active)}
\def\ucprimaryscenario{
\begin{enumerate}
...
\def\ucsummary{Global preferences are application properties that can be changed by the user (e.g. i18n/language)}
\def\ucpreconditions{Jake is running (GEN-01)}
\def\uctriggers{User action (active)}
\def\ucprimaryscenario{
\begin{enumerate}
